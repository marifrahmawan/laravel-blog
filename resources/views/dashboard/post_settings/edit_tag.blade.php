            ...
            <div class="row">
                <div class="col">
                    <div class="card col-6 shadow">
                        <div class="card-body">
                            <form action="{{ route('update-tag', $tag->slug) }}" method="POST">
                                @csrf
                                @method('patch')

                                <div class="form-group col">
                                    <label for="name">Name</label>
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') ?? $tag->name}}" autofocus autocomplete="off">
                                    @error('name')
